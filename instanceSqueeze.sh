...
if [ "$EUID" -eq 0 ]; then
    ...
    log_error "Run it as a standard user with sudo privileges: ./instanceSqueeze.sh install ..."
    exit 1
fi
...
show_command_help() {
    ...
    case "$cmd" in
        ...
        completion)
            ...
            echo "    echo 'source <( /path/to/instanceSqueeze.sh completion generate )' >> ~/.bashrc"
            echo "    ./instanceSqueeze.sh completion generate >> ~/.bash_completion"
            ;;
        *)
...
# WARNING: THIS FILE IS AUTOGENERATED BY instanceSqueeze.sh
...
EOF
    ...
    log_success "Run './instanceSqueeze.sh restart $APP_NAME' to apply changes."
}
...
complete -F _instancesqueeze_completion instanceSqueeze.sh ./instanceSqueeze.sh instanceSqueeze
EOF
}
