...
This guide details the complete developer lifecycle for developing an application locally on a workstation, packaging it into a container image, pushing it to the GitHub Container Registry (`ghcr.io`), and deploying it seamlessly to your production VM using the `appSqueezer.sh` tool.
...
```bash
./appSqueezer.sh create-app ghcr.io/yourusername/my-service:latest
```
